...
def domain_to_bits(x, domain=DOMAIN, bits=BITS):
    """expects input tensor ranging from 0 to `domain`, outputs bit tensor ranging from -1 to 1"""
    device = x.device
...
def bits_to_domain(x, domain=DOMAIN, bits=BITS):
    device = x.device
...
class BitDiffusion(nn.Module):
    ...
    def forward(self, img, *args, **kwargs):
        ...
        img = domain_to_bits(img) * self.bit_scale
...
def sudoku_reward(x):
    ...
    max_violations = (block_size * size) * (size - 1)

    rewards = []

    for b in range(batch):
        v = sudoku_violations(x[b, 0])
        rewards.append(1 - v / max_violations)

    return torch.tensor(rewards, device=x.device)


def sudoku_violations(x):
    ...
    v = 0
    for c in range(size):
        v += size - torch.unique(x[:, c]).shape[0]

    for r in range(size):
        v += size - torch.unique(x[r, :]).shape[0]

    for r in range(0, size, block_size):
        for c in range(0, size, block_size):
            v += size - torch.unique(x[r : r + block_size, c : c + block_size]).shape[0]
...
class SudokuDataset(Dataset):
    def __init__(
        self, instance_file, augment_horizontal_flip=False, augment_vertical_flip=False
    ):
        super().__init__()
        ...
        self.transform = T.Compose(
            [
                T.RandomApply([fliplr]) if augment_horizontal_flip else nn.Identity(),
                T.RandomApply([flipud]) if augment_vertical_flip else nn.Identity(),
            ]
        )
            ...
            while self.step < self.train_num_steps:
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        save_samples(
                            all_images,
                            str(self.results_folder / f"sample-{milestone}.txt"),
                        )
                        self.save(milestone)
...
def save_samples(samples, path):
    ...
    with open(path, "w") as f:
        for s in samples.cpu():
            ...
            violations.append(v)
            print(s.numpy(), file=f)
            print(f"violations={v}", file=f)
            print("----", file=f)
...
if __name__ == "__main__":
    ds = SudokuDataset("sudoku.npy")

    x = torch.stack([ds[i] for i in range(1)])
    print(x.shape)
    print(sudoku_reward(x[:,:,0:9, 0:9]))
